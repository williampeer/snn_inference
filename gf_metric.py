...
# def get_gamma_factor(model, target, time_interval, dt, rate_correction=True):
#     r'''
#     Calculate gamma factor between model and target spike trains,
#     with precision delta.
#     Parameters
...
#     # model_rate = model_length / time
#     # data_rate = data_length / time
#     model_rates = [len(model) / float(time_interval) for i, times in enumerate(model.values())]
#     target_rates = [len(data) / float(time_interval) for i, times in enumerate(target.values())]
#
#     if len(model) > 1:
#         bins = .5 * (model[1:] + model[:-1])
#         indices = np.digitize(data, bins)
