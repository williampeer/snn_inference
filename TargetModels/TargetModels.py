...
def lif_HS_17_continuous_ensembles_model_dales_compliant(random_seed, N = 12):
    ...
    pop_size = int(N / 3)
    params_pop1 = {'tau_m': 6., 'E_L': -42., 'tau_s': 7.0}
    weights_std = 0.1
    # weights_std = 0
    hand_coded_params_pop1 = {'preset_weights': (torch.ones((pop_size, 1)) +
                                                 (2*weights_std * torch.randn((pop_size, N))) - weights_std) *
    ...
    params_pop2 = {'tau_m': 5., 'E_L': -55., 'tau_s': 5.}
    ...
    params_pop3 = {'tau_m': 4., 'E_L': -70., 'tau_s': 3.}
    hand_coded_params_pop3 = {'preset_weights': (torch.ones((pop_size, 1)) +
                                                 (2*weights_std * torch.randn((pop_size, N))) - weights_std) *
...
def lif_r_continuous_ensembles_model_dales_compliant(random_seed, N = 12):
    ...
    hand_coded_params_pop2 = {'preset_weights': (torch.ones((pop_size, 1)) +
                                                ...
                                                # torch.cat([T(pop_size * [0.0]), T(pop_size * [0.0]), T(pop_size * [0.0])])}
                                                torch.cat([T(pop_size * [.1]), T(pop_size * [.4]), T(pop_size * [.2])])}
...
def lif_asc_continuous_ensembles_model_dales_compliant(random_seed, N = 12):
    ...
    pop_size = int(N / 3)
    params_pop1 = {'tau_m': 7., 'E_L': -55., 'f_I': 0.45, 'G': 0.9, 'delta_theta_s': 18., 'b_s': 0.3, 'f_v': 0.14,
                   'delta_V': 12., 'tau_s': 6.}
    weights_std = 0.1
    # weights_std = 0
    hand_coded_params_pop1 = {'preset_weights': (torch.ones((pop_size, 1)) +
    ...
    params_pop2 = {'tau_m': 5.8, 'E_L': -58., 'f_I': 0.4, 'G': 0.85, 'delta_theta_s': 16., 'b_s': 0.3, 'f_v': 0.14,
    ...
    hand_coded_params_pop2 = {'preset_weights': (torch.ones((pop_size, 1)) +
                                                ...
                                                # torch.cat([T(4*[.0]), T(4*[.0]), T(4*[0.0])])}
                                                torch.cat([T(pop_size * [.1]), T(pop_size * [.35]), T(pop_size * [.18])])}

    params_pop3 = {'tau_m': 4.3, 'E_L': -68., 'f_I': 0.35, 'G': 0.9, 'delta_theta_s': 18., 'b_s': 0.25, 'f_v': 0.14,
                   'delta_V': 12., 'tau_s': 3.}
    hand_coded_params_pop3 = {'preset_weights': (torch.ones((pop_size, 1)) +
                                                 (2*weights_std * torch.randn((pop_size, N))) - weights_std) *
                                                ...
                                                # torch.cat([T(4*[-.0]), T(4*[-.0]), T(4*[-0.0])])}
                                                torch.cat([T(pop_size * [-.1]), T(pop_size * [-.15]), T(pop_size * [-.25])])}
...
def lif_r_asc_continuous_ensembles_model_dales_compliant(random_seed, N = 12):
    ...
    hand_coded_params_pop3 = {'preset_weights': (torch.ones((pop_size, 1)) +
                                                ...
                                                # torch.cat([T(pop_size * [-.4]), T(pop_size * [-.2]), T(pop_size * [-0.02])])}
                                                # torch.cat([T(4*[-.0]), T(4*[-.0]), T(4*[-0.0])])}
                                                torch.cat([T(pop_size * [-.04]), T(pop_size * [-.08]), T(pop_size * [-.12])])}
...
def glif_continuous_ensembles_model_dales_compliant(random_seed, N = 12):
    ...
    params_pop1 = {'tau_m': 3.4, 'G': 0.7, 'E_L': -52., 'delta_theta_s': 18., 'b_s': 0.4, 'f_v': 0.14, 'delta_V': 10.,
                   'f_I': 0.5, 'b_v': 0.3, 'a_v': 0.2, 'theta_inf': -12., 'tau_s': 4.8}
    weights_std = 0.1
    # weights_std = 0
    hand_coded_params_pop1 = {'preset_weights': (torch.ones((pop_size, 1)) +
                                                 (2*weights_std * torch.randn((pop_size, N))) - weights_std) *
    ...
    params_pop2 = {'tau_m': 3., 'G': 0.7, 'E_L': -58., 'delta_theta_s': 14., 'b_s': 0.3, 'f_v': 0.14, 'delta_V': 12.,
                   'f_I': 0.4, 'b_v': 0.35, 'a_v': 0.25, 'theta_inf': -14., 'tau_s': 3.5}
    hand_coded_params_pop2 = {'preset_weights': (torch.ones((pop_size, 1)) +
                                                 (2*weights_std * torch.randn((pop_size, N))) - weights_std) *
                                                # torch.cat([T(4*[.02]), T(4*[.3]), T(4*[0.15])])}
                                                # torch.cat([T(pop_size * [.08]), T(pop_size * [.38]), T(pop_size * [0.32])])}
    ...
    params_pop3 = {'tau_m': 2.6, 'G': 0.8, 'E_L': -66., 'delta_theta_s': 18., 'b_s': 0.25, 'f_v': 0.14, 'delta_V': 12.,
                   'f_I': 0.35, 'b_v': 0.4, 'a_v': 0.3, 'theta_inf': -11., 'tau_s': 2.}
    hand_coded_params_pop3 = {'preset_weights': (torch.ones((pop_size, 1)) +
                                                 (2*weights_std * torch.randn((pop_size, N))) - weights_std) *
                                                # torch.cat([T(pop_size * [-.4]), T(pop_size * [-.2]), T(pop_size * [-0.02])])}
                                                # torch.cat([T(pop_size * [-.4]), T(pop_size * [-.2]), T(pop_size * [-0.02])])}
