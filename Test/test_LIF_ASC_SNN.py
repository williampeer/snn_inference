...
static_parameters = {'N': 12, 'w_mean': 0.1, 'w_var': 0.3}
free_parameters = {'tau_m': 2.0, 'tau_g': 2.0, 'v_rest': -60.0}

snn = LIF_ASC(device='cpu', parameters=zip_dicts(static_parameters, free_parameters))
...
plot_neuron(membrane_potentials.data, title='LIF_ASC neuron plot ({:.2f} spikes)'.format(spikes.sum()), fname_ext='test_LIF_ASC_poisson_input')

zeros = torch.zeros_like(inputs)
membrane_potentials_zeros, spikes_zeros = model_util.feed_inputs_sequentially_return_spikes_and_potentials(snn, zeros)
plot_neuron(membrane_potentials_zeros.data, title='LIF_ASC neuron plot ({:.2f} spikes)'.format(spikes_zeros.sum()), fname_ext='test_LIF_ASC_no_input')

plot_spiketrains_side_by_side(spikes, spikes_zeros, 'test_LIF_ASC', title='Test LIF_ASC spiketrains random and zero input')
