...
for random_seed in range(3, 4):
    ...
    snn = TargetModelsSoft.lif_pop_model(random_seed=random_seed, pop_size=pop_size, N_pops=num_pops)

    inputs = experiments.sine_modulated_white_noise(t=5000, N=snn.N, neurons_coeff=torch.cat([T(int(snn.N/2) * [0.]), T(int(snn.N/4) * [0.5]), T(int(snn.N/4) * [0.])]))
    # inputs = experiments.strong_sine_modulated_white_noise(t=5000, N=snn.N, neurons_coeff=torch.cat([T(int(snn.N / 2) * [1.]), T(int(snn.N / 2) * [0.])]))
    # inputs = experiments.sine_input(t=5000, N=snn.N, neurons_coeff=torch.cat([T(int(snn.N / 4) * [1.]), T(int(snn.N / 4) * [0.]), T(int(snn.N / 2) * [0.])]))
