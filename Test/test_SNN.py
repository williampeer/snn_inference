...
for random_seed in range(1, 6):
    ...
    inputs = continuous_normalised_poisson_noise(10., t=8000, N=snn.N)  # now assumes rate in Hz
    ...
    plot_spiketrains_side_by_side(spikes, spikes_zero_weights, 'test_LIF', title='Test LIF spiketrains random input', legend=['Random weights', 'No weights'])
    print('avg. rate: {}'.format(spikes.sum() / (spikes.shape[1] * 12)))
