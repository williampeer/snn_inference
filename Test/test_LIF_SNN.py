...
plot_neuron(membrane_potentials.data, title='LIF_test neuron plot ({:.2f} spikes)'.format(spikes.sum()), fname_ext='test_LIF_poisson_input')

zeros = torch.zeros_like(inputs)
membrane_potentials_zeros, spikes_zeros = model_util.feed_inputs_sequentially_return_spikes_and_potentials(snn, zeros)
plot_neuron(membrane_potentials_zeros.data, title='LIF_test neuron plot ({:.2f} spikes)'.format(spikes_zeros.sum()), fname_ext='test_LIF_no_input')

plot_spiketrains_side_by_side(spikes, spikes_zeros, 'test_LIF', title='Test LIF_test spiketrains random and zero input')
