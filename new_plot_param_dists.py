...
def plot_param_dist_across_exp_combined(fitted_param_dists, init_param_dists, fname, lr):
    ...
    for folder_path in folders:
        ...
        if model_type is None:
            ...
            for f in files:
                ...
                fname = 'export_param_dist_fitted_{}_exp_{}.png'.format(folder_path, exp_num)
                dist_per_param = plot_param_dist(model, tar_model, fname=fname, lr=lr)

                init_fname = 'export_param_dist_initial_{}_exp_{}.png'.format(folder_path, exp_num)
                init_dist_per_param = plot_param_dist(init_model, tar_model, fname=init_fname, lr=lr)
            ...
            fname_exp = 'export_param_dist_exp_avg_exp_{}.png'.format(folder_path)
            ...
            if results_dict.keys().__contains__(config_key):
                results_dict[config_key]['fitted'].append(np.mean(param_dist_per_param_fitted))
                results_dict[config_key]['init'].append(np.mean(param_dist_per_param_init))
            else:
                results_dict[config_key] = { 'fitted' : [np.mean(param_dist_per_param_fitted)],
                                             'init' : [np.mean(param_dist_per_param_init)] }
